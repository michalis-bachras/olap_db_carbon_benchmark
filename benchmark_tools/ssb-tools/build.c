...
// Function to remove '-' and convert to integer
int removeDashAndConvertToInt(const char *odate,size_t odate_size) {

    // Check for an empty string
    if (odate_size == 0) {
        fprintf(stderr, "Error: Empty string\n");
        return -1; // You might want to handle this error condition appropriately
    }

    // Allocate memory for a temporary string without dashes
    char *temp = malloc(odate_size + 1);
    if (temp == NULL) {
        fprintf(stderr, "Error: Memory allocation failed\n");
        return -1; // Handle memory allocation failure
    }

    // Remove dashes from the odate array
    size_t j = 0;
    for (size_t i = 0; i < odate_size; i++) {
        if (odate[i] != '-') {
            temp[j++] = odate[i];
        }
    }
    ...
    // Convert the temporary string to an integer
...
mk_order(long index, order_t *o, long upd_num)
	...
	for (lcnt = 0; lcnt < o->lines; lcnt++)
		...
		//printf("o->odate: %s\n", o->odate);
        // Get the size of the odate array
        size_t odate_size = strlen(o->odate);
        int odate_int = removeDashAndConvertToInt(o->odate, odate_size);
        o->lineorders[lcnt].orderdate = odate_int;
        //printf("Result: %d\n", odate_int);
		...
		c_date += tmp_date;
        size_t commit_date_size = strlen(asc_date[c_date - STARTDATE]);
        int commit_date_int = removeDashAndConvertToInt(asc_date[c_date - STARTDATE], commit_date_size);
        o->lineorders[lcnt].commit_date = commit_date_int;
...
/*
P_NAME is as long as 55 bytes in TPC-H, which is un\ACreasonably large. 
We reduce it to 22 by limiting to a concatena\ACtion of two colors (see [TPC-H], pg 94). 
