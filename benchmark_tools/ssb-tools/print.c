...
pr_line(order_t *o, int mode)
{
	...
	PR_INT(fp_l, o->lineorders[i].partkey);
        ...
        PR_INT(fp_l, o->lineorders[i].orderdate);
	...
	//PR_STR(fp_l, o->lineorders[i].commit_date, DATE_LEN);
            PR_INT(fp_l, o->lineorders[i].commit_date);
...
int pr_date(date_t *d, int mode){
    ...
    PR_INT(d_fp,atoi(d->lastdayinweekfl));
    PR_INT(d_fp,atoi(d->lastdayinmonthfl));
    PR_INT(d_fp,atoi(d->holidayfl));
    PR_INT_LAST(d_fp,atoi(d->weekdayfl));
...
vrf_line(order_t *o, int mode)
{
    ...
    for (i = 0; i < o->lines; i++)
        ...
        VRF_INT(LINE, o->lineorders[i].orderdate);
	   ...
	   // VRF_STR(LINE, o->lineorders[i].commit_date);
        VRF_INT(LINE, o->lineorders[i].commit_date);
